...
def default_source_weight(source_name: str, section: str) -> int:
    ...
    if any(source in name for source in ("physics world", "aps physics")):
        return 5
    if any(source in name for source in ("cern", "nasa", "esa", "isro")):
        return 4
    if section == "india" and any(source in name for source in ("iisc", "iit", "tifr", "pib", "science")):
        return 3
    return 1
...
def keyword_set(text: str) -> set[str]:
    ...
    stopwords = {
        ...
        "science", "physics", "research", "study", "studies", "scientists",
        "researchers", "says", "said", "finds", "found", "could", "first",
...
- The SUMMARY must not copy the first bullet. It should combine 2 to 4 themes, for example: "Quantum devices, Indian space science, and astrophysics results."
- Keep SUMMARY under 160 characters.
- Then produce exactly two sections:
  SECTION: India Physics
...
- When both sections have strong candidates, include at least one India item and one World item. Do not force a weak item.
...
- Prioritize items useful for physics students: conceptual clarity, experiments, measurements, space missions, major facilities, and discoveries.
- Prioritize items useful for research direction: emerging themes, new instruments, unusual measurements, major collaborations, and significant papers.
- Keep ordinary institutional announcements only if they clearly affect physics research or education.
...
- Do not merge unrelated discoveries, missions, papers, policy announcements, or institutional updates just because they share a broad theme.
...
- Prefer concrete, teachable physics news over hype, speculation, or routine press releases.
- End each bullet with source ids using this exact format: Sources: [I1], [I3] or Sources: [W2]
- Do not include inline URLs.
- Format bullets as: - **Short topic:** one concise synthesized sentence explaining what happened and why it matters. Sources: [I1], [W3]
...
def generic_title(title: str) -> bool:
    ...
    generic_titles = {
        "physics brief",
        "physics news brief",
        "daily physics news brief",
        "india and world physics brief",
    }
...
def one_line_summary(summary: str, items: list[NewsItem]) -> str:
    ...
    return "Daily physics news for students and research direction"
...
def build_post(summary: str, items: list[NewsItem], used_ai: bool, points_per_section: int) -> Path:
    ...
    post_path = POSTS / f"{now_ist.date().isoformat()}-physics-brief.md"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the daily physics news brief.")
    ...
    api_key = os.environ.get("PHYSICS_API_KEY")
